<?php
/**
 * FF MVC PHP 微框架 (/index.php/ctrl_class/ctrl_class_method/param/value/...).
 *
 * 支持 URL 常见写法: 
 ...
 * @var string $m  模型对象集
 * @var string $l  类库对象集
 * @var array  $d  视图 d 的额外数据集, 方便在派生 C 类中为视图增加变量
 * @var array  $ws WebService 连接
 */
class FF
{
    public $m  = array();
 ...
 *
 * @var array $sql 调试信息, 记录 SQL 语句
 * @var array $db  数据库连接实例集
 */
class M extends FF
{
    public $sql = array();
     ...
     *     'order_by'   => "",
     *     // 返回: array(array('field1' => value1), ...)
     *     // one 为真时返回第一条记录的数据数组: array('field1' => value1, ...)
     *     'one'        => 0,
     *     // 为真是返回记录总数, 分页时必须为真
     *     'total'      => 0
     * );
     * // 条件为键(键可包含操作符, 与字段名以空格分隔)值方式, 自动采用预处理语句查询
     * // 所有条件均为 AND, 特殊需求可采用附加条件参数处理
     * $where = array();
     * $pages = array(
 ...
 * 视图表现层 (V).
 *
 * V 视图, 即 v 目录下的文件命名与 C 控制器派生类及方法对应, 如: 
 * user_list.php 对应 class user extends C { function _list() {} }
 *
